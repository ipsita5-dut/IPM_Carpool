...
module.exports.registerDriver = async function(req, res) {
  ...
  try {
      let { dName,dEmail,dcontact,license, dAddress, latitude,longitude, password ,carType,vehicleNo,isAvailableNextday}= req.body;
      ...
      let newDriver = await driverModel.create({
        ...
        vehicleNo,
        isAvailableNextday: isAvailableNextday === 'true', // Convert to boolean
...
module.exports.getPendingRideRequests = async function(req, res) {
  try {
      ...
      for (const request of rideRequests) {
        ...
        request.destinationAddress = res[0] ? res[0].formattedAddress : 'Address not found'; // Store the formatted address
      }
